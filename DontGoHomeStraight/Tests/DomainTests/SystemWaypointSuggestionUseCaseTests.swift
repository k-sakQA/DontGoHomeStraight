...
final class SystemWaypointSuggestionUseCaseTests: XCTestCase {
    ...
    struct DMStub: DistanceMatrixClient {
        ...
        func getDurationsSeconds(origin: CLLocationCoordinate2D, destinations: [CLLocationCoordinate2D], mode: TransportMode) async throws -> [TimeInterval] {
            if destinations.count == 1 { return toDurations }
            return toDurations + toDestDurations + [baseline]
        }
    }
    ...
    final class PlaceRepoStub: PlaceRepository {
        ...
        func searchPlaces(location: CLLocationCoordinate2D, type: String, radius: Int) async throws -> [Place] {
            defer { callIndex += 1 }
            return callIndex < placesByCall.count ? placesByCall[callIndex] : []
        }
        ...
        func getPhotoURL(photoReference: String, maxWidth: Int) -> URL? { nil }
    }
    
    struct CacheStub: CacheRepository {
        func getExcludedPlaceIds() async -> [String] { [] }
        func addExcludedPlaceId(_ placeId: String) async {}
        func clearExcludedPlaceIds() async {}
        func savePlacesForGenres(places: [Place], genres: [Genre]) async {}
        func getPlaceForGenre(_ genre: Genre) async -> Place? { nil }
        func saveSpotDetails(_ place: Place, forGenre genre: Genre) async {}
        func getSpotDetails(forGenre genre: Genre) async -> Place? { nil }
        func getCachedRecommendations(key: String) async -> [Genre]? { nil }
        func cacheRecommendations(_ genres: [Genre], forKey key: String) async {}
        func clearCache() async {}
    }
    
    struct AIRepoStub: AIRecommendationRepository {
        func getRecommendations(for input: RecommendationInput) async throws -> [LLMCandidate] { [] }
        func generateHint(for input: PlaceHintInput) async throws -> String {
            return "テストヒント"
        }
    }
    
    private func makePlace(id: String, name: String = "TestPlace", type: String, rating: Double, reviews: Int) -> Place {
        Place(
            name: name,
            coordinate: CLLocationCoordinate2D(latitude: 0, longitude: 0),
            address: "TestAddress",
            genre: Genre(name: "TestGenre", category: .other, googleMapType: type),
            rating: rating,
            priceLevel: 1,
            photoReference: nil,
            isOpen: true,
    ...
    func testBasicPick() async throws {
        ...
        let usecase = SystemWaypointSuggestionUseCase(placeRepository: repo, distanceMatrixClient: dm, cacheRepository: CacheStub(), aiRepository: AIRepoStub())
    ...
    func testBoundaryOver30MinutesExcluded() async throws {
        ...
        let usecase = SystemWaypointSuggestionUseCase(placeRepository: repo, distanceMatrixClient: dm, cacheRepository: CacheStub(), aiRepository: AIRepoStub(), config: cfg)
        let genres = try await usecase.getRecommendations(currentLocation: CLLocationCoordinate2D(latitude: 0, longitude: 0), destination: CLLocationCoordinate2D(latitude: 1, longitude: 1), mood: Mood(activityType: .outdoor, vibeType: .discovery), transportMode: .walking, now: Date(timeIntervalSince1970: 0), seed: "s")
